...
void AHICameraManager::CameraMode(float DeltaTime)
{
    if (currentCamView != CameraView::SCRIPTED)
    {
        ...
        {
            player->springArm->TargetArmLength = FMath::Lerp(springArmLengInitRot, distanceFromPlayer, lerpCurve->GetFloatValue(interpAlpha));
        }
    ...
    else
    {
        if (interpAlpha == 0.f)
        {
            werewolfController = UHIGameData::HIGetPlayerController();// Cast<AHIWerewolfController>(player->GetController());
        }
...
void AHICameraManager::HISweepActors(TArray<FHitResult>& _hitResult, ECollisionChannel _collisionChannel /*= ECollisionChannel::ECC_GameTraceChannel2*/)
{
    if (hiWorld)
    {
